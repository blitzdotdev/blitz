...
describe('scene journal', () => {
    it('appends JSONL and filters by date and count', async () => {
        const root = await mkdtemp(resolve(tmpdir(), 'kite3d-journal-'))
        roots.push(root)
    ...
    it('does not append an entry for an empty semantic summary', async () => {
        const root = await mkdtemp(resolve(tmpdir(), 'kite3d-journal-empty-'))
        roots.push(root)
        ...
        await expect(readFile(resolve(root, '.kite3d/journal.jsonl'), 'utf8')).rejects.toMatchObject({code: 'ENOENT'})
    })
})
