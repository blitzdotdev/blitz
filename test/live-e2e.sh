...
WORK_DIR=$(mktemp -d)
TRANSCRIPT=${TRANSCRIPT:-/tmp/kite3d-phase-b-e2e-transcript.txt}
...
const moduleUrl = pathToFileURL(resolve(repository, 'packages/kite3d/src/indexHtml.ts')).href
...
build_files() {
  ...
  while IFS= read -r -d '' FILE_PATH; do
    REL=${FILE_PATH#./}
    case "/$REL/" in */.kite3d/*|*/.git/*|*/node_modules/*|*/dist/*|*/.*/*) continue ;; esac
...
await page.waitForFunction(() => (window.__kite3dUpdates || 0) > 0)
await page.waitForFunction(() => Boolean(
  window.__kite3dViewer?.scene.modelRoot.getObjectByName('PropMesh'),
))
const first = await page.evaluate(() => window.__kite3dUpdates || 0)
await page.waitForFunction((before) => (window.__kite3dUpdates || 0) > before, first)
const result = await page.evaluate(() => ({
  updates: window.__kite3dUpdates || 0,
  propMesh: Boolean(window.__kite3dViewer?.scene.modelRoot.getObjectByName('PropMesh')),
}))
