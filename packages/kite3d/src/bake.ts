...
export function checkBakeSafety(
    ...
    const realChildren = (target.node.children || [])
        ...
        .filter((node) => node && node.extras?.kite3dGenerated !== true)
    ...
    const bakedFrom = target.node.extras?.kite3dBakedFrom
...
function isHumanClient(client: string): boolean {
    return client !== 'external' && client !== KITE3D_SERVER_CLIENT_ID && client !== 'kite3d-bake'
}
