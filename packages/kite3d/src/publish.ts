...
export async function publishProject({
    ...
    try {
        ...
        if (!registeredHashes.includes(runtimeHash)) {
            console.warn(`[kite3d] Installed runtime ${runtimeHash} differs from registered ${version} runtime ${registered.sha256}; publishing the installed runtime.`)
        }
    } catch (error) {
        if (isHttpNotFound(error)) {
            console.warn(`[kite3d] Runtime ${version} is not registered; publishing the installed runtime. A strict backend may reject this release.`)
        } else {
            const detail = sanitizeDiagnostic(error instanceof Error ? error.message : error, [
                entry.deploy_token,
                entry.claim_secret,
            ])
            console.warn(`[kite3d] Could not compare runtime ${version} with the registry: ${detail}. Publishing the installed runtime.`)
        }
    }
...
export async function pullProject({
    ...
    if (!slug) throw new Error('The deploy entry is not present in .kite3d/deploys.json.')
...
function packageDisplayName(packageJson: Record<string, unknown>, fallback: string): string {
    ...
    if (kite3d && typeof kite3d === 'object' && !Array.isArray(kite3d)) {
        const configured = (kite3d as Record<string, unknown>).name
...
export function publishExcludes(packageJson: Record<string, unknown>): string[] {
    const kite3d = packageJson.kite3d
    if (!kite3d || typeof kite3d !== 'object' || Array.isArray(kite3d)) return []
    const publish = (kite3d as Record<string, unknown>).publish
    if (publish === undefined) return []
    if (!publish || typeof publish !== 'object' || Array.isArray(publish)) {
        throw new Error('package.json kite3d.publish must be an object.')
    }
    ...
    if (!Array.isArray(exclude) || exclude.some((value) => typeof value !== 'string')) {
        throw new Error('package.json kite3d.publish.exclude must be an array of glob strings.')
    }
...
async function usePinnedRuntimeVersion(dirHandle: FileSystemDirectoryHandle, packageJson: Record<string, unknown>): Promise<{
    ...
    if (!devDependencies || typeof devDependencies !== 'object' || Array.isArray(devDependencies)) {
        throw new Error('package.json must pin kite3d in devDependencies.')
    }
    const spec = (devDependencies as Record<string, unknown>)['kite3d']
    ...
    const rawKite3d = packageJson.kite3d
    if (rawKite3d !== undefined && (!rawKite3d || typeof rawKite3d !== 'object' || Array.isArray(rawKite3d))) {
        throw new Error('package.json kite3d must be an object.')
    }
    const kite3d = rawKite3d ? {...rawKite3d as Record<string, unknown>} : {}
    if (kite3d.version !== undefined && typeof kite3d.version !== 'string') {
        throw new Error('package.json kite3d.version must be a string.')
    }
    const changed = kite3d.version !== version
    if (changed) kite3d.version = version
    return {
        packageJson: changed ? {...packageJson, kite3d} : packageJson,
        version,
        changed,
    }
}
