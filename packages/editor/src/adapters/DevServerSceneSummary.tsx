...
export function DevServerSceneSummary() {
    ...
    const sceneFile = manager.manifest.find(({path}) => path === manager.scenePath)
    const cameraName = findSceneCameraName(manager.get().scene.modelRoot)
    ...
    return <div className="kite3d-inspector-empty">
        <div className="kite3d-empty-prompt">
            ...
            <span>Click an object in the viewport or the Objects list.</span>
        </div>
        <section className="kite3d-panel-section kite3d-scene-summary" data-testid="scene-summary">
            <header className="kite3d-section-header"><h3>Scene</h3></header>
...
function findSceneCameraName(root: {traverse(callback: (object: {isCamera?: boolean, name?: string}) => void): void}) {
    let name = ''
    root.traverse((object) => {
        if (!name && object.isCamera) name = object.name?.trim() || 'Camera'
    })
    return name
}
