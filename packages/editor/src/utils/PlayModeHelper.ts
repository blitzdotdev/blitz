...
export class PlayModeHelper extends EventDispatcher<{
    ...
    private readonly pauseKey = Symbol('editor-play-pause')
    ...
    async pauseRunMode() {
        const manager = this.manager
        if (!this.isRunningMode || this.isPausedRunning) return
        this.isPausedRunning = true
        const viewer = manager.get()
        viewer.timeline.stop()
        viewer.getPlugin(EntityComponentPlugin)?.disable(this.pauseKey)
        viewer.getPlugin(CannonPhysicsPlugin)?.disable(this.pauseKey)
    ...
    async unpauseRunMode(startTime = true) {
        ...
        if (physics) {
            // Cannon's fixedStep uses wall time; discard the paused interval.
            physics.world.lastCallTime = undefined
            physics.world.accumulator = 0
            physics.enable(this.pauseKey)
        }
        viewer.getPlugin(EntityComponentPlugin)?.enable(this.pauseKey)
        if (startTime) viewer.timeline.start()
        this.dispatchEvent({type: 'runModePauseChange'})
    }
