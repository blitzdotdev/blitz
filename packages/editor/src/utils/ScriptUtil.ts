    ...
    private async refLoadModule(mod: ScriptModule){
        ...
        Object.entries(module).forEach(([key, exp])=>{
            ...
            if((exp as ComponentRef['exp']).ComponentType){
                ...
                if(e &&
                    !components.find(r=>r.def === e && r.exp === componentCons) &&
                    !newComp.find(r=>r.def === e && r.exp === componentCons)) {
                    newComp.push({
                        exp: componentCons,
                        def: e,
                    })
        ...
        // Registration mutates plugin/component registries and is not
        // re-entrant. Preserve package.json/module export order instead of
        // racing all classes with Promise.all.
        const results: boolean[] = []
        for (const p of newRefs) {
