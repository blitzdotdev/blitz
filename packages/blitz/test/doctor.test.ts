...
describe('blitz doctor', () => {
    ...
    it('fails the version-pin row when the project and command differ', async () => {
        ...
        manifest.devDependencies['@blitzdev/blitz'] = '9.9.9'
    ...
    it('accepts a live server for this project and reports its pid and age', async () => {
        ...
        await mkdir(resolve(fixture.root, '.blitz'), {recursive: true})
        await writeFile(resolve(fixture.root, '.blitz/dev.json'), JSON.stringify({
            pid: process.pid,
            port: address.port,
...
async function readyProject(options: {registeredRuntime?: boolean, git?: boolean, parentGit?: boolean} = {}) {
    ...
    for (const name of ['blitz', 'editor', 'engine', 'template']) {
        ...
        await writeFile(resolve(packageRoot, 'package.json'), JSON.stringify({version: BLITZ_VERSION}))
    }
