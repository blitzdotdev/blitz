...
export async function registerRuntime({rootDirectory = repositoryDirectory, fetchImplementation = fetch} = {}) {
    const environment = await loadReleaseEnvironment(rootDirectory)
    ...
    const engineDirectory = resolve(rootDirectory, 'packages/engine')
    ...
    const response = await fetchImplementation(`${backendUrl}/api/v1/runtimes/${encodeURIComponent(packageJson.version)}`, {
        method: 'PUT',
        headers: {
