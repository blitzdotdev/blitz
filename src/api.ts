...
export interface Kite3dApiOptions {
    baseUrl: string
    gameId?: string
    token?: string
...
export class Kite3dApiError extends Error {
    readonly name = 'Kite3dApiError'
...
export class Kite3dApi {
    ...
    async hasBlob(hash: string): Promise<boolean> {
        try {
            ...
            if (error instanceof Kite3dApiError && error.status === 404) return false
            throw error
        }
    }
    ...
    private async rawRequest(
        ...
        for (let attempt = 1; attempt <= attempts; attempt += 1) {
            try {
                ...
                const safeError = error instanceof Kite3dApiError ? error : transportError(init.method || 'GET', error)
                if (!isTransientError(safeError) || attempt === attempts
                    || (!(safeError instanceof Kite3dApiError) && !isRetryableMethod(init.method, path))) throw safeError
                lastError = safeError
...
async function apiError(response: Response): Promise<Kite3dApiError> {
    ...
    return new Kite3dApiError(response.status, code, message, details, retryAfter(response.headers.get('Retry-After')))
}
...
function transportError(method: string, error: unknown): Error {
    ...
    return Object.assign(new Error(`${method.toUpperCase()} request lost its connection (${sanitizeDiagnostic(code)}).`), {
        name: 'Kite3dTransportError',
        transient: true,
    })
}

function isTransientError(error: unknown): boolean {
    return error instanceof Kite3dApiError
...
function retryDelay(error: unknown, attempt: number): number {
    const requested = error instanceof Kite3dApiError ? error.retryAfterMs : undefined
