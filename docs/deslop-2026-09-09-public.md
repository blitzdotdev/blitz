# Public Kite3D deslop pass - 2026-09-09
...
| 3 / S4 | `packages/kite3d/src/publish.ts` | APPLIED | Deleted the second upload pool and routed uploads through `Kite3dApi.uploadBlobs`. | 26 |
...
| 6 / S9 | `packages/kite3d/src/publish.ts` | APPLIED | Deleted `PublishApi` and structural fakes; publish accepts `Kite3dApi`, and tests exercise the real client through the mock HTTP backend. | 219 |
...
| 9 / S7 | `packages/kite3d/src/server.ts` | APPLIED | Deleted the unused asset-library proxy option, environment/default, state field, editor type, and DOM attribute. | 10 |
| 10 / S7 | `packages/kite3d/src/publish.ts` | APPLIED | Reduced publish targeting to the only produced form, a required slug, and derives the stored deploy entry. | 39 |
...
| 14 / S4 | `.github/workflows/ci.yml` | APPLIED | Removed duplicate focused publish and release validation runs; the full Kite3D suite and `release.mjs` remain the owners. | 7 |
| 15 / S9 | `packages/kite3d/src/commands.ts` | APPLIED | Deleted command/install/runtime injection options; extracted pure migration selection and retained a real upgrade fixture path. | 51 |
...
| 21 / S1 | `packages/template/template/package.json` | APPLIED | Removed empty plugin, script, and viewer settings; the template emits only `kite3d.version`. | 4 |
...
| 24 / S9 | `packages/kite3d/src/server.ts` | APPLIED | Deleted test-only token/editor-directory options; tests use the returned token and built editor. | 21 |
...
| 27 / S7 | `packages/kite3d/src/commands.ts` | APPLIED | Deleted the legacy string form of `publishFromDisk`; callers pass `PublishFromDiskOptions`. | 8 |
...
|---|---:|---:|
| `npm install --ignore-scripts --cache /tmp/kite3d-npm-cache` | PASS | 19 s |
...
| `npm test -w packages/kite3d` | PASS - 53 tests | 4.66 s |
...
| `npx eslint packages/kite3d/src/commands.ts packages/kite3d/src/index.ts packages/kite3d/src/publish.ts packages/kite3d/src/server.ts packages/kite3d/test/mockBackend.ts packages/kite3d/test/publish.test.ts packages/kite3d/test/server.test.ts packages/kite3d/test/upgrade.test.ts packages/editor/src/App.tsx packages/editor/src/DevServerSource.ts packages/editor/src/PublishDialog.tsx packages/editor/src/publishing.ts packages/editor/src/vite-env.d.ts packages/editor/test/editor/dev-server.spec.ts packages/engine/src/plugins/GeneratorComponent.ts packages/engine/src/runtime/createGame.ts packages/engine/src/runtime/migrations.ts packages/engine/src/runtime/projectFormat.ts packages/engine/src/sceneSerialization.ts packages/engine/test/unit/generator.test.ts` | PASS - 0 errors | 1.63 s |
...
```text
 ...
 package.json                                      |   2 +-
 packages/kite3d/src/commands.ts                    |  37 ++-
 packages/kite3d/src/index.ts                       |   7 +-
 packages/kite3d/src/publish.ts                     |  75 +-----
 packages/kite3d/src/server.ts                      |  22 +-
 packages/kite3d/test/mockBackend.ts                |  61 ++++-
 packages/kite3d/test/publish.test.ts               | 307 +++++++---------------
 packages/kite3d/test/server.test.ts                |  18 +-
 packages/kite3d/test/upgrade.test.ts               | 111 +++++---
...
db9464d deslop(kite3d): remove duplicate publish and server seams
