...
kite3d publish          # walks, hashes, uploads, releases; prints the live URL
kite3d pull             # downloads the active release into the folder
...
The editor runs only from `kite3d dev`. There is no editor on any domain and no follow mode. Publishing and claiming happen from the local editor through the local server, or from the command line. blitz.dev is the store, see `docs/storefront-plan.md`.

## Agents

`agents.md` says: run `kite3d init`, run `kite3d dev`, print the editor URL, edit files, run `kite3d publish`, print the live URL. Pull before publish. Read `.kite3d/journal.jsonl` after the human has been editing. Requirement: Node 20 or newer.

## Phases

**C1. `kite3d`, server, editor source, packages.** `kite3d` with `init`, `dev`, `publish`, `pull`, `open`. `packages/engine` extracted, `packages/editor` moved to `packages/editor`, and the starter moved to `packages/kite3d/template`. The editor on `DevServerSource`, the deletions above, play mode through `createGame`. Tests: server unit and integration, and an editor Playwright suite that runs against a real dev server. The editor E2E becomes fully automatable, because no folder picker is involved.
...
- Phase B part 2 as specified in `docs/publish-dialog.md`: the dialog stays, the folder walk in the browser goes; `kite3d publish` walks.
