# Integrating the ai-game-prototype branch into Kite3D
...
- Their companion is our `kite3d dev`. Same loopback role and capability model. Our server already replaces their folder-permission pairing, which they list as an awkward deferred problem.
...
### P1. Authoring contract, validator, and `kite3d check`
...
- Engine: `packages/engine/src/authoring.ts` with `userData.kite3dAuthoring` `{role: 'direct'|'template'|'generator', id, sourceId, allowCameraInside}`; `RuntimeObjectOwner` with `attachRuntimeRoot`, `trackEffect`, `cloneFrom`, `cleanup`; runtime roots live outside `modelRoot` and are removed on Stop. Extend `GeneratorComponent` with the bounded, non-multiplying preview rule.
...
- CLI and server: `kite3d check` runs the report through the open editor (server asks over SSE, the editor runs and posts the result) or headless through the runtime; results to `.kite3d/check.json` and the console. Playable, Editable, Persisted stay separate outcomes.
- Template: distill their 12-line AGENTS contract into ours: the three patterns, runtime roots outside `modelRoot`, `start()` repeatable and `stop()` mandatory, no play state in the saved scene, camera framed on every load, run `kite3d check` before calling a change done.
...
- `kite3d publish`: refuse during Play or with an unsaved editor draft (from `.kite3d/state.json`), save the scene first, reject symlinks, lock duplicate attempts, retry 429 and 5xx with uncertain-write reconciliation, verify every public asset byte for byte by SHA-256 from the live URL after release, strip tokens from diagnostics, keep status across reloads.
- Tests: their conflict, rate-limit, timeout, connection-loss, uncertain-write, and hash-mismatch matrix in `packages/kite3d/test/publish.test.ts` against `mockBackend.ts`.
...
- `kite3d doctor`: Node version, free port, pin match, packages installed, backend reachable.
- `kite3d checkpoint <label>` and `kite3d restore`: git-based, with an editor button. One checkpoint before agent work, one restore.
- `kite3d archive`: the sanitized source ZIP with a provenance file. Later.
...
- Crystal Vault is not ported. Kite3D ships its own samples.
- Routine calls: `kite3d init` creates a git repo so checkpoints are commits; `kite3d check` runs through the open editor first, headless later; hot reload uses a server-side import rewrite with a real ES module lexer; telemetry and validation hooks ship with P1; Node 20 or newer.
