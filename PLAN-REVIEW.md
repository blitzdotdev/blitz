...
- RUNNING: CLI onboarding pass (`cli-onboarding`): the no-argument output, `init`, `dev`, and the root guard carry the whole loop (init, npm install, read AGENTS.md, dev, check, publish) so the prompt "use npx @blitzdev/blitz and build me X" is enough; then a patch release 0.12.1.
