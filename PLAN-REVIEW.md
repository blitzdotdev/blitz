...
- DECIDED (owner): the maker tool is renamed Kite3D; the platform stays Blitz (blitz.dev, *.app.blitz.dev, the publish API, `_blitz/runtime.js`). New names: npm `kite3d` (bare CLI, `npx kite3d`), `@kite3d/engine`, `@kite3d/editor`, `@kite3d/template`; project key `kite3d`, state dir `.kite3d/`, asset URL form `/kite3d//`; `kite3d upgrade` migrates legacy projects. blitzdotdev/blitz stays as a mirror; the primary repo becomes kite3d/kite3d once the owner confirms the org. The kite is the only logo (editor, README, template game icon).
