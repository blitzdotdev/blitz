...
- DECIDED (owner): plugins are plain npm packages, no CLI machinery. Convention: keyword `kite3d-plugin`, a README as the agent guide, an entry export the engine loads, a peer dependency on `@blitzdev/engine`; use with `npm install` plus a `kite3d.plugins` entry; find with `npm search kite3d-plugin`. The guide gets one paragraph (after the layout pass, ships in 0.14.0). PR #3 review verdict: keep its command policy table, drop `kite3d skills`.
