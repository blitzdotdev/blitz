...
| Rename Kite to Kite3D; MCP bridge and in-editor AI chat removed; `.kite3d/` gitignore paths fixed | BUILT | commit `42fa7a9`; tsc and vite build pass |
...
| NOW-1 as rebuilt by C1: `kite3d dev` local server watches the folder, serves files with hashes and `If-Match`, streams change events with writer ids; the editor uses one `DevServerSource`; File System Access, IndexedDB, service worker, fetch proxy deleted; play mode through `createGame` | BUILT, merged | merge `0baecd5`; suites: 12 server tests, editor Playwright against a real `kite3d dev`, runtime, publish; my manual run of init, dev, PUT, SSE, 401, traversal |
| NOW-1 IndexedDB | DONE by deletion | no IndexedDB remains in the editor |
| Version pinning | BUILT differently | the project pins `kite3d` as a devDependency; `npx kite3d` runs that version; no bootstrapper |
| Journal and agent mirrors | BUILT | journal in C2a; `.kite3d/state.json` and `.kite3d/console.log` written by the editor through the dev server (C1) |
| NOW-2 as revised (C2a): deterministic text glTF at `mainScene` with external `.bin` and hashed textures, validation with errors to `.kite3d/console.log`, `Generator` component with generated children excluded from export, scoped `kite3d bake` with refusal rules, `.kite3d/journal.jsonl` semantic diff of human edits, `kite3d journal`, agent docs | BUILT, merged | commits `52b73b7`..`1a49f93`; 19 server tests, 2 editor Playwright tests, runtime test |
...
| Open-source split: `@blitzdev/engine`, `@blitzdev/editor`, `kite3d`, `@blitzdev/template`, Apache-2.0, `src/` shipped | PUBLISHED 0.12.0 on npm (2026-09-10, tag `v0.12.0`) | packaging proof: `npm pack` + local install + `kite3d init` + `kite3d dev` + `kite3d sources` (threepipe src present); publishing to npm is one command on your go |
| `services/asset-library-proxy` deployed for the editor's asset library | UNBUILT | worker not created |
| Kite3D logo and artwork | UNBUILT | editor still shows the kite artwork |
...
| Live agent path with the real command: `kite3d init`, `kite3d publish` (8 files, release, live URL), headless load shows the viewer with zero console errors | BUILT, proven | run by the reviewer on 2026-09-09 late evening against the sandbox backend; test game deleted afterwards |
...
| Pass E: versions from manifests (`/api/state` reports kite3d, editor, engine), `KITE3D_SERVER_CLIENT_ID` constant, `kite3d init` copies the whole template with stamped versions, version pin rule (delegate to the project's installed `kite3d`, refuse when missing, `KITE3D_IGNORE_VERSION_PIN` bypass), `kite3d publish` uses the project pin, `kite3d upgrade` with migrations and journal, dev server on Hono `LinearRouter`, gateway accepts local editor origins, asset library proxy deployed | BUILT, merged | merge `837f968`; 34 server tests, 8 publish, runtime, 3 editor; asset proxy deployed |
...
- DONE: fix pass G (merge `0855660`): all 15 run-2 findings plus the F residuals fixed, `kite3d check` (paths, imports, component types, generators) gates publish, `kite3d status` shows the live dev server, credentials hidden from file APIs, viewport chrome parity restored, AGENTS template consolidated with the verified gotchas, `docs/agents.md` generated from it. 62 CLI tests, 7 editor Playwright tests.
- DONE: P1 (merge `08df708`): `packages/engine/src/authoring.ts` (roles direct, template, generator; `RuntimeObjectOwner`), `authoringValidation.ts` (quality, cleanup, persistence reports with the failure codes; 12-fixture matrix), `kite3d check` with Playable, Editable, Persisted headless or through the open editor, result cards in the editor, AGENTS rules. Verified on a copy of FPS Drill: all three outcomes pass.
...
- DONE: P6 (merge `9a7f06d`): `kite3d doctor`, `kite3d init` creates a git repo, `kite3d checkpoint` and `kite3d restore` with editor buttons and server routes, `kite3d archive`; 101 CLI tests, 17 editor tests. Known nit: doctor reports a busy default port as FAIL although `kite3d dev` picks a free port.
- DONE: E2E run 3 with a clean-memory agent on tarballs from `f2eea7d`: FPS Range built, `kite3d check` passed Playable, Editable, Persisted in both modes, published and verified live; the agent reported six gaps (script paths without `./` in the editor, the editor check validating the viewport camera instead of the saved one, viewer settings not applied in the editor, journal noise on key order, `dirty` stuck for 15 s after a check, non-listed module hot reload which P2 fixes). Session review running.
...
- DONE: fix pass I (merge `3180c17`): all eleven run-4 findings, the H6 residual (unreferenced uuids dropped on save), and a 41-line engine quick reference in the AGENTS template; `kite3d init` inside a parent repository now creates its own repository and checkpoint/restore refuse a parent repository unless `--allow-parent-repo`. Two more vendored threepipe changes to send upstream (`DirectionalLight2.ts`, `SpotLight2.ts`: light targets excluded from export). Tarballs repacked, runtime `f021ac17` registered, guide `17b35da9` uploaded.
...
- DONE: fix pass L (merge `e455682`): project commands refuse to run outside a Kite3D project root (the owner ran `kite3d dev` in the parent folder and got "Cannot read assets.json: 404"); the editor treats a missing `assets.json` as empty; Open game moves into the play button group as an icon button. Root rule: the current directory must hold a package.json with an kite3d dependency and assets/main.scene.gltf; parents are not searched. Tests: 8 commands in two invalid folders fail before and pass after; the editor loads a project without assets.json and creates it on the first drop. Suites: kite3d 123, editor 29. Tarballs repacked from `e455682`.
...
- DONE: npm release v0.12.0 from `da34313`: @blitzdev/engine, template, editor, kite3d published public; runtime registered; guide uploaded; tag pushed. Registry reads lagged about 80 seconds. Proof from a clean folder: `npx kite3d init`, `npm install`, `kite3d doctor`, and a headless `kite3d check` pass. The paste block in docs/testing-with-an-agent.md now uses the public command.
- DONE: CLI onboarding (merge `bbe284a`, release v0.12.1 from `7cd60e3`): the no-argument output, `init`, `dev`, and the root guard carry the whole loop (init, npm install, read AGENTS.md, dev, check, publish) so the prompt "use npx kite3d and build me X" is enough; then a patch release 0.12.1. Verified from the registry: `npx kite3d` with no argument prints the workflow first.
- DONE: stale-install pass (release v0.12.2): `kite3d dev` and the no-argument output print the Kite3D version first; `init` on an existing project answers "already a Kite3D project, cd <dir> && npx kite3d dev"; `doctor` warns on a file: or link: pin; the folder guard lists child project folders. Cause: the owner ran `npx kite3d dev` in ~/kite3d-games, whose stale package.json and node_modules held the 0.12.0 tarballs, so npx ran the old CLI and served the parent folder.
...
- DECIDED (owner): the maker tool is renamed Kite3D; the platform stays Blitz (blitz.dev, *.app.blitz.dev, the publish API, `_blitz/runtime.js`). New names: npm `kite3d` (bare CLI, `npx kite3d`); `@blitzdev/engine`, `@blitzdev/editor`, and `@blitzdev/template` keep their names; project key `kite3d`, state dir `.kite3d/`, asset URL form `/kite3d/@`; `kite3d upgrade` migrates legacy projects. The repository is renamed in place to blitzdotdev/kite3d. The kite is the only logo (editor, README, template game icon).
- RUNNING: the Kite3D rename pass (`kite3d-rename`), version 0.13.0.
- DONE: editor parity measurement (`d44e248`, `docs/editor-parity-2026-09-10.md`) against Brandon's branch at `9a3c7c2`: 84 files and 460 hunks differ; 0 of 40 screenshot states match (5 to 9 percent per state, the Timeline tab crashes the editor at 99 percent). Cause: the earlier restoration used Kite3D's own commit `a80b8ae` as the baseline, never his branch, and later passes rewrote `ThreeEditorComponent.tsx`, `renderer.scss`, the panels, the inspector, the files and library panels, the welcome dialog, and unmounted the context menu.
- DONE: parity restoration (merge `1bd2eca`): reference files byte for byte, agreed differences as adapters (no Prompt Mode or AI chat, no MCP or companion, the three Kite3D controls and publish dialog, DevServerSource), re-measured to 0 percent. The owner wants no standing golden-screenshot guard: this is a one-time restoration and the editor may change afterwards, so the `test:parity` part of the running pass is dropped before the merge. Result: 39 of 40 screenshot states at 0.0000 percent; the Project panel differs by 117 pixels, the threepipe version label (0.5.1 here, 0.4.4 there), proven by a scratch 0.4.4 build. The Memory tab stays (it is the asset registry view, not AI). Agreed differences live in `packages/editor/src/adapters/`. All suites pass on main (editor 28 of 28). Tarballs repacked from `1bd2eca`; runtime and guide hashes unchanged.
...
Build order agreed (revised, evening): Phase A done, Phase B part 1 running, then C1 kite3d command, local dev server, editor source, then C2 cloud source, dialog, glTF model. See `docs/local-dev-server-plan.md`.
...
- So "add the engine version in manifest" means: add it under the `kite3d` block in package.json. After the rename the block, the URL prefix, and the folder all become `kite3d`.
...
- Cheapest replacement, no protocol: the editor writes read-only mirror files into `.kite3d/`. `state.json` (editor version, play state, selection, last load error) and `console.log` (runtime console and errors, rotated). Agents read them after each edit. The scene hierarchy needs no mirror. The scene file is the hierarchy.
...
4. **Validation on load with visible errors.** Write load errors into `.kite3d/console.log`. An agent must learn when its edit broke the file.
...
5. **Claim secret by default.** Right after creation, call `authorize-claim-secret`. Store `{project_id, tp_token, claim_secret, preview_url, expires_at}` in `.kite3d/deploys.json`, gitignored. The "open game" button reads `preview_url` from it. On sign-up the editor claims every project in the file. Sign-up alone preserves nothing.
...
7. **Slugs are global across teenyapp and kite3d.** A collision returns 409. Rules: 3 to 49 chars, lowercase, digits, hyphens, no `--`. `backend/src/utils/validation.ts:6`
...
3. NOW-1: manifest = the `kite3d` block in package.json, exact version, bootstrapper at blitz.dev.
4. NOW-1: journal = file-level reverse patches at save granularity; in-memory stack stays as a cache; add `.kite3d/state.json` and `.kite3d/console.log` mirrors.
...
- The deploy secrets live in `.kite3d/deploys.json`, gitignored, with `claim_secret` for later sign-in (section 5.6 there, item 5 in section 3 here).
...
5. **Backend additions are in progress here:** `GET /api/v1/slugs/:slug`, the spinner branch (503 with `X-Kite3D-State`), the `runtimes` table and routes with `RUNTIME_UPLOAD_TOKEN`, GC roots for runtimes, and Google sign-in with the existing teenyapp client id. Blob GC itself was a gap in the shipped backend and is being built now (refcounts, release retention, grace-period sweep, bounded reconciliation).
...
10. **Open-source split like PlayCanvas (evening decision).** Engine, editor, the `kite3d` command, and the template are open (Apache-2.0) npm packages that ship their `src/`, so a project's `node_modules` holds everything an agent may grep. The cloud services stay closed. Unlike PlayCanvas, the editor runs standalone from `kite3d dev` with no cloud. There is no hosted editor and no follow mode. Plan: `docs/open-source-split.md`.

11. **Layout agreed (evening).** Public `blitzdotdev/kite3d` with `packages/engine`, `editor`, `kite3d`, `template`; private `blitzdotdev/blitz-cloud` from `services/` with history. Apache-2.0 confirmed by the owner for the editor. Exact engine contents in `docs/layout.md`.
