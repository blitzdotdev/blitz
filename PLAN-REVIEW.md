...
- DECIDED (owner): the maker tool is renamed Kite3D; the platform stays Blitz (blitz.dev, *.app.blitz.dev, the publish API, `_blitz/runtime.js`). New names: npm `kite3d` (bare CLI, `npx kite3d`); `@blitzdev/engine`, `@blitzdev/editor`, `@blitzdev/template` keep their names (no new npm org); project key `kite3d`, state dir `.kite3d/`, asset URL form `/kite3d//`; `kite3d upgrade` migrates legacy projects. The repository is renamed in place to blitzdotdev/kite3d (GitHub redirects the old name). The kite is the only logo (editor, README, template game icon).
...
- DECIDED (owner): layout. `packages/uiconfig-blueprint` moves into the editor package (editor-only vendored renderer); `packages/template` moves into the CLI package as `packages/kite3d/template/` and `@blitzdev/template` stops being a package; `packages/threepipe` stays (shared by engine and editor). Ships with the migration fix as 0.14.0. Layout pass staged behind `upgrade-fix3`.
