...
- RUNNING: fix pass L (`fix-l`): project commands refuse to run outside a Blitz project root (the owner ran `blitz dev` in the parent folder and got "Cannot read assets.json: 404"); the editor treats a missing `assets.json` as empty; Open game moves into the play button group as an icon button.
- DECIDED (owner, Option A): Check becomes an icon button right of Open game with a pass or fail dot and a hover popover of the three outcomes; Checkpoint and Restore last checkpoint become Save Scene menu items; the centered toolbar group goes away. Fix pass M is staged behind L.
