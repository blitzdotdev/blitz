...
- RUNNING: parity restoration (`parity-restore`): reference files byte for byte, agreed differences as adapters (no Prompt Mode or AI chat, no MCP or companion, the three Blitz controls and publish dialog, DevServerSource), re-measured to 0 percent, plus a golden-screenshot `test:parity` guard.
