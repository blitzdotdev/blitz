...
- RUNNING: fix pass J for E7, E8, P9 and the E6 row test (`fix-j`); the cloud follow-up that simplifies the limiter to native shield then D1 counter, adds the slug-check limit if documented, and makes the integration suite green (`ratelimit-deslop`).
