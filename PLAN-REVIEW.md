...
- RUNNING in parallel: P2 transitive hot reload (`p2-hot-reload`), P6 doctor, git init, checkpoint and restore, archive (`p6-cli`), P4 source editing in the Inspector as a view (`p4-source-editor`), and E2E run 3 on the current tarballs with a fresh agent (project `fps-range`).
