...
- DONE: `@blitzdev/plugin-mujoco` 0.1.0 published (peer and test dependency on engine 0.14.0, repo at `b952f9c`). Legacy proof on a copy of fps-shooter: `npx -y kite3d@0.14.0 upgrade` took it from 0.12.0 to 0.14.0, doctor packages PASS, runtime PASS, check Playable, Editable, Persisted PASS.
...
- RUNNING: `ci-green`: fix the 79 editor lint errors at the source, run lint in the release preflight, and skip publishing versions that already exist so the tag workflow becomes a green verification run.
